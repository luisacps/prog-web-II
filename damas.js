...
function dragstart() {
    document.addEventListener("dragstart", function(event) {event.dataTransfer.setData("Text",event.target.id)
        posicaoInicial = event.path[1].idclass = (event.path[0].className)  })
}

function dragend() {
    document.addEventListener("dragend", function(event) {event.preventDefault()   })
}
